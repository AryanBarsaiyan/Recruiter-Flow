...
/**
 * Feature tests: start interview with invitation token (public).
 */
class InterviewFeatureTest extends AbstractIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        recruiterEmail = "int-" + UUID.randomUUID() + "@test.com";
        candidateEmail = "int-cand-" + UUID.randomUUID() + "@test.com";
        ...
        testDataHelper.createCandidateProfile(candidateUser, "Int Candidate");
    ...
    private String signupAndGetToken() throws Exception {
        String body = objectMapper.writeValueAsString(Map.of(
                ...
                "password", "password1",
                "fullName", "Rec",
                "companyName", "Int Co " + UUID.randomUUID().toString().substring(0, 8)));
    ...
    private UUID createJobViaApi(String token, UUID companyId) throws Exception {
        String body = objectMapper.writeValueAsString(Map.of("companyId", companyId.toString(), "title", "Int Job"));
    ...
    @Test
    void startInterview_withAcceptedInvitation_returnsInterviewAndQuestions() throws Exception {
        String body = objectMapper.writeValueAsString(Map.of("invitationToken", invitationToken));
        mockMvc.perform(post("/interviews/start")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isOk())
